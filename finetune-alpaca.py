...
def main():
    ...
    model_name = "bigscience/bloom-560m" #model_args.model_name_or_path
    tokeniser = BloomTokenizerFast.from_pretrained(
        f"{model_name}", add_prefix_space=True
    )
    model = BloomForCausalLM.from_pretrained(f"{model_name}", torch_dtype=torch.float8_e5m2fnuz).to(device)
